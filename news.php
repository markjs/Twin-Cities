...
function acquireNews($searchTerm) {
	$config = loadConfig();
    $returnContent="";
	var_dump((string)$config['news']['numberOfResults']);
	//assembles the request URI to the API
    echo $newsUri = $config['news']['baseUri'] . "?format=xml" . "&q=" . urlencode($searchTerm) . "&order-by=" . urlencode($config['news']['orderBy']) . "&page-size=" . $config['news']['numberOfResults'] . "&date-id=" . urlencode("date/" . $config['news']['dates']);
	
    $newsData = json_decode(acquire_file($newsUri));
